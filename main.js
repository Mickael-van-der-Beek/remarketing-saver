...
function writeFirstOfPathReport (report) {
  document.getElementById('first-of-path-report').innerHTML = `
    ...
    In ${beautifyInteger(report.firstOfPath.size)} (${beautifyFloat(report.sampling.size && (report.firstOfPath.size / (report.sampling.size / 100)), '%')}) of these transactions, ${beautifyWord(adSource)} retargeting ads appeared as the <strong><u>first step of the path</u></strong>
    ...
    Your potential savings using our optimisation: ${beautifyFloat(report.sampling.size && (report.firstOfPath.size / (report.sampling.size / 100)), '%')} of your ${beautifyWord(adSource)} remarketing budget!
  `.trim();
}
...
function writeMiddleOfPathReport (report) {
  document.getElementById('middle-of-path-report').innerHTML = `
    In ${beautifyInteger(report.sampling.size)} transactions (${beautifyFloat(report.sampling.rate * 100, '%')} sample rate),
    ${beautifyWord(adSource)} retargeting ads appeared in the customer's conversion path between ${beautifyDate(report.query.startDate)} and ${beautifyDate(report.query.endDate)}.
    In ${beautifyInteger(report.middleOfPath.size)} (${beautifyFloat(report.sampling.size && (report.middleOfPath.size / (report.sampling.size / 100)), '%')}) of these transactions, ${beautifyWord(adSource)} retargeting ads appeared as <strong><u>intermediate steps</u></strong>
    ...
    <br />
    ${beautifyWord(adSource)} will account for these types of conversion paths which are considered to be inefficient!
    Your potential savings using our optimisation: ${beautifyFloat(report.sampling.size && (report.middleOfPath.size / (report.sampling.size / 100)), '%')} of your ${beautifyWord(adSource)} remarketing budget!
  `.trim();
}
...
function writeLastOfPathReport (report) {
  document.getElementById('last-of-path-report').innerHTML = `
    ...
    a total of ${beautifyInteger(report.lastOfPath.size)} (${beautifyFloat(report.sampling.size && (report.lastOfPath.size / (report.sampling.size / 100)), '%')}) were at least attributed once in the <strong><u>last step of the path</u></strong>
...
function writeContactSection (report) {
  document.getElementById('contact-hook').innerHTML = `
    If you want to save ${beautifyFloat(report.sampling.size && ((report.firstOfPath.size + report.middleOfPath.size) / (report.sampling.size / 100)), '%')}
